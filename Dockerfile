...
RUN cd internal/persistence && cargo build --release

# Stage 2: Build Zig shared library
FROM ubuntu:22.04 AS zig-builder
RUN apt-get update && apt-get install -y curl xz-utils && rm -rf /var/lib/apt/lists/*
RUN curl -L https://ziglang.org/builds/zig-linux-x86_64-0.14.0.tar.xz | tar -xJ -C /usr/local \
    && ln -s /usr/local/zig-linux-x86_64-0.14.0/zig /usr/local/bin/zig
WORKDIR /app
COPY internal/protocol ./internal/protocol
COPY --from=rust-builder /app/internal/storage/target/release/librust_storage.a ./internal/storage/target/release/librust_storage.a
...
WORKDIR /app
COPY --from=zig-builder /app/internal/protocol/zig-out/lib/libprotocol.so ./internal/protocol/zig-out/lib/libprotocol.so
